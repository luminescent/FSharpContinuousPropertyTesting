...
[<Property>]    
let  ``returns Fizz when evenly divisible by 3`` d =
    (d % 3 = 0 && d % 5 <> 0) ==> ("Fizz" = fizzBuzz d)

[<Property>]        
let ``returns Buzz when evenly divisible by 5`` d =
    (d % 5 = 0 && d % 3 <> 0) ==> ("Buzz" = fizzBuzz d)

[<Property(Arbitrary = [| typeof<IntsEvenlyDivisibleBy3And5>|])>]
//[<Property>]
let ``returns FizzBuzz when evenly divisible by 3 and 5`` d =
    (d % 3 = 0 && d % 5 = 0) ==> ("FizzBuzz" = fizzBuzz d)

[<Property>]
let ``returns number when not evenly divisible by 3 nor 5`` d = 
